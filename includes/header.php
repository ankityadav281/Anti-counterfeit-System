...
<head>
    ...
    <script>
        window.APP_CONFIG = {
            googleClientId: <?php echo json_encode(app_env('GOOGLE_CLIENT_ID', '')); ?>,
            appEnv: <?php echo json_encode(app_env('APP_ENV', 'local')); ?>,
            appDebug: <?php echo json_encode(filter_var(app_env('APP_DEBUG', 'true'), FILTER_VALIDATE_BOOLEAN)); ?>,
            googleLoginUrl: <?php echo json_encode(app_url('auth/google-login.php')); ?>
        };
    </script>
</head>
<body>
